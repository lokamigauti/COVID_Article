...
```{r}
i = 1
models <- data.frame(city = NA, k = NA, k_sd = NA, k_t = NA, k_p = NA, v = NA, v_sd = NA, v_t = NA, v_p = NA, pm_25 = NA, pm_10 = NA, uv = NA, t = NA, density = NA, population = NA)
  ...
  for(nlag in lags){
    data_training <- ds %>%
      ...
      summarise(uvb = mean(uvb), 
                t2m = mean(t2m), 
                median_pm25 = mean(median_pm25), 
                median_pm10 = mean(median_pm10), 
...
# Falta fazer as médias acima ignorarem os na e calcular os r^2 pra cada par lag mvavg
