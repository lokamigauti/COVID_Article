...
for(city in cities){
  ...
  tryCatch(currentmodel <- nls(covid_cases ~ (population*k/(1+exp(5 + v*day))), data = data_training, start = list(k = 0.01, v = -1), control = list(maxiter = 1000)), error = function(e) NA)
  ...
  if(is.na(currentmodel) == TRUE) {
    ...
    res <- nlsResiduals(currentmodel)
    test.nlsResiduals(res) #how can I extract the p value from the shapiro, no of the runs?
    
    plot(data_training$day, data_training$covid_cases, xlim = c(0, 30), main = city)
    lines(data_training$day, predict(currentmodel, newdata = data_training))
  }
...
models_rsqr %>%
  filter(lag == 12, mvavg == 1) %>%
  ggplot(aes(y = v, x = pm_25, label = city))+
  geom_point(aes(color = v_sd), size = 5)+
  geom_abline(intercept = 0.13, slope = 0.002)+
  scale_color_viridis()
  # geom_errorbar(aes(ymin = v - v_sd, ymax = v + v_sd))+
  # geom_errorbarh(aes(xmin = pm_25 - pm_25_sd, xmax = pm_25 + pm_25_sd))+
  #geom_smooth(method = "lm", se = FALSE)+
  #geom_text(aes(label=city),hjust=-0.06, vjust=-0.5)+
  xlab("Average Daily Particulate Matter") + ylab("COVID 19 Cases' Growth Rate")
