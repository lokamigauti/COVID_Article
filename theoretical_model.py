...
def run_simulation(particulate, dt=1/24, ndays=90, popsize=1e6, removal_rate=1e-3, infection_rate=2e-2):
    nt = int(ndays / dt)
    I = np.zeros(nt)
    R = np.zeros(nt)
    S = np.zeros(nt)
    I[0] = 1  # first individual
    R[0] = 0  # removed
    S[0] = popsize
    alpha = removal_rate
    ...
    for t in range(nt - 1):
        ...
        I[t + 1] = I[t] + dt * (beta[t]*S[t]*I[t] -alpha*I[t])
...
control_R, control_I, control_S, beta_control = run_simulation(particulate_control,
                                                 dt=dt, ndays=ndays, infection_rate=0.5e-6,
                                                 removal_rate=0.5e-1)

R, I, S, beta = run_simulation(particulate, dt=dt, ndays=ndays, infection_rate=0.5e-6, removal_rate=0.5e-1  )
